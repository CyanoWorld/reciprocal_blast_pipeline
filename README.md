...
### Some information for coding

Views are triggered in combination with the urls.py and views.py file. HTTP POST methods are connected to form validation. Form validation is defined in the forms.py file. Database transactions and file uploads are handled in the service.py file. Snakemake execution with previous configuration setup is handled in the blast_execution.py file. The project_creation view executes functions that are in the project_creation.py file which utilize functions of the service.py file. 
