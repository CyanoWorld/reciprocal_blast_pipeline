...
## Installation
In order to start developing and using this application you need to download the application from the git-repository. There are two possibilities for installing the application either you install the package distribution platform [anaconda](https://www.anaconda.com/) or the container virtualization platform [docker](https://www.docker.com/). Installation via docker involves alteration and editing of the docker-compose.yaml file with subsequent execution of `docker-compose up`. Particularly one would have to change the path of the volumes into: 
